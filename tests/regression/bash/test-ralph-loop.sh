...
extract_functions() {
    ...
    sed -n '/^validate_commit_subject()/,/^}/p' "$SOURCE_SCRIPT"
    sed -n '/^is_recoverable_commit_subject_postcondition()/,/^}/p' "$SOURCE_SCRIPT"
}
...
TMP_SUBJECT_REPO="$TMP_GATE_ROOT/subject-repair"
cp -R "$TMP_ACTIVE_REPO" "$TMP_SUBJECT_REPO"
TMP_SUBJECT_SPEC="$TMP_SUBJECT_REPO/specs/test-feature"
mkdir -p "$TMP_SUBJECT_REPO/.specify/extensions/ralph"
cat > "$TMP_SUBJECT_REPO/.specify/extensions/ralph/ralph-config.yml" << 'SUBJECTCONFIG'
model: "gpt-4o"
max_iterations: 5
agent_cli: "copilot"
commit:
  ...
  issue: "auto"
SUBJECTCONFIG
printf '%s\n' '- [ ] T001 Complete work' > "$TMP_SUBJECT_SPEC/tasks.md"
cp "$FIXTURE_DIR/ralph-memory-valid-active.md" "$TMP_SUBJECT_SPEC/ralph-memory.md"
printf '%s\n' 'active subject validation run' >> "$TMP_SUBJECT_SPEC/progress.md"
git -C "$TMP_SUBJECT_REPO" add .
git -C "$TMP_SUBJECT_REPO" commit -qm "test: active subject validation baseline"
git -C "$TMP_SUBJECT_REPO" checkout -qb 069-feature

SUBJECT_CALLS="$TMP_GATE_ROOT/subject.calls"
set +e
subject_output=$(cd "$TMP_SUBJECT_REPO" && RALPH_TEST_CALLS="$SUBJECT_CALLS" RALPH_TEST_MODE=subject-bad-then-repair RALPH_TEST_SPEC="$TMP_SUBJECT_SPEC" RALPH_TEST_COMPLETE_MEMORY="$FIXTURE_DIR/ralph-memory-valid-complete.md" bash "$SOURCE_SCRIPT" --feature-name "test-feature" --tasks-path "$TMP_SUBJECT_SPEC/tasks.md" --spec-dir "$TMP_SUBJECT_SPEC" --max-iterations 2 --model "fake-model" --agent-cli "$ACTIVE_AGENT" 2>&1)
subject_exit=$?
set -e
assert_eq "bad commit subject is repairable by next iteration" "0" "$subject_exit"
assert_true "bad commit subject reports commit-subject-invalid" grep -q '^commit-subject-invalid:' <<< "$subject_output"
assert_eq "bad commit subject invokes repair iteration" "2" "$(wc -l < "$SUBJECT_CALLS" | tr -d ' ')"
...
before_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
assert_true "accepts substantive commit containing all state artifacts" validate_iteration_commit_history "$TMP_COMMIT_REPO" "$before_head" "$before_state" "$before_ids" "$before_completed_ids" 0 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md"
...
failed_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
assert_true "accepts failed-attempt retention without HEAD advance" validate_iteration_commit_history "$TMP_COMMIT_REPO" "$failed_head" "$failed_state" "$failed_ids" "$failed_completed_ids" 7 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md"
...
followup_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
assert_true "accepts later substantive inclusion of retained records" validate_iteration_commit_history "$TMP_COMMIT_REPO" "$followup_head" "$followup_state" "$followup_ids" "$followup_completed_ids" 0 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md"
...
review_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
assert_true "accepts coordinated state-only commit when task state advances" validate_iteration_commit_history "$TMP_COMMIT_REPO" "$review_head" "$review_state" "$review_ids" "$review_completed_ids" 0 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md"
...
expansion_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
assert_true "accepts substantive task-expansion commit" validate_iteration_commit_history "$TMP_COMMIT_REPO" "$expansion_head" "$expansion_state" "$expansion_ids" "$expansion_completed_ids" 0 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md"
...
state_expansion_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
bookkeeping_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
set +e
bookkeeping_output=$(validate_iteration_commit_history "$TMP_COMMIT_REPO" "$bookkeeping_head" "$bookkeeping_state" "$bookkeeping_ids" "$bookkeeping_completed_ids" 0 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md" 2>&1)
bookkeeping_exit=$?
set -e
...
task_add_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
set +e
task_add_output=$(validate_iteration_commit_history "$TMP_COMMIT_REPO" "$task_add_head" "$task_add_state" "$task_add_ids" "$task_add_completed_ids" 0 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md" 2>&1)
task_add_exit=$?
set -e
...
printf '%s\n' '- [x] T015 Stale completed duplicate' '- [ ] T015 Open duplicate' '- [ ] T016 Open task' >> "$TMP_COMMIT_SPEC/tasks.md"
git -C "$TMP_COMMIT_REPO" add "$TMP_COMMIT_SPEC/tasks.md"
git -C "$TMP_COMMIT_REPO" commit -qm "prepare duplicate stale completion"
duplicate_stale_head=$(get_git_head_snapshot "$TMP_COMMIT_REPO")
duplicate_stale_state=$(get_task_state_snapshot "$TMP_COMMIT_SPEC/tasks.md")
duplicate_stale_ids=$(get_incomplete_task_ids "$TMP_COMMIT_SPEC/tasks.md")
duplicate_stale_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
printf '%s\n' '- [ ] T017 Add-only duplicate follow-up' >> "$TMP_COMMIT_SPEC/tasks.md"
printf '%s\n' '- duplicate stale completion should not count' >> "$TMP_COMMIT_SPEC/ralph-memory.md"
printf '%s\n' 'duplicate stale completion should not count' >> "$TMP_COMMIT_SPEC/progress.md"
git -C "$TMP_COMMIT_REPO" add "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/ralph-memory.md" "$TMP_COMMIT_SPEC/progress.md"
git -C "$TMP_COMMIT_REPO" commit -qm "duplicate stale add-only churn"
set +e
duplicate_stale_output=$(validate_iteration_commit_history "$TMP_COMMIT_REPO" "$duplicate_stale_head" "$duplicate_stale_state" "$duplicate_stale_ids" "$duplicate_stale_completed_ids" 0 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md" 2>&1)
duplicate_stale_exit=$?
set -e
assert_eq "duplicate stale checked task validation exits one" "1" "$duplicate_stale_exit"
assert_true "duplicate stale checked task is not counted as progress" grep -q '^failed-iteration-advanced-head:' <<< "$duplicate_stale_output"
...
dirty_pre_agent_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
set +e
dirty_pre_agent_output=$(validate_iteration_commit_history "$TMP_COMMIT_REPO" "$dirty_pre_agent_head" "$dirty_pre_agent_state" "$dirty_pre_agent_ids" "$dirty_pre_agent_completed_ids" 0 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md" 2>&1)
...
multi_completed_ids=$(get_completed_task_ids "$TMP_COMMIT_SPEC/tasks.md")
...
set +e
multi_output=$(validate_iteration_commit_history "$TMP_COMMIT_REPO" "$multi_head" "$multi_state" "$multi_ids" "$multi_completed_ids" 0 "$TMP_COMMIT_SPEC/tasks.md" "$TMP_COMMIT_SPEC/progress.md" "$TMP_COMMIT_SPEC/ralph-memory.md" 2>&1)
multi_exit=$?
set -e
assert_eq "multi-commit state-only checkpoint validation exits one" "1" "$multi_exit"
assert_true "earlier checkpoint cannot borrow later task completion" grep -q "^bookkeeping-only: commit $checkpoint_commit " <<< "$multi_output"
...
partial_completed_ids=$(get_completed_task_ids "$TMP_PARTIAL_SPEC/tasks.md")
...
assert_true "accepts partial-story subset committed as coordinated work unit" validate_iteration_commit_history "$TMP_PARTIAL_REPO" "$partial_head" "$partial_state" "$partial_ids" "$partial_completed_ids" 0 "$TMP_PARTIAL_SPEC/tasks.md" "$TMP_PARTIAL_SPEC/progress.md" "$TMP_PARTIAL_SPEC/ralph-memory.md"
...
second_completed_ids=$(get_completed_task_ids "$TMP_PARTIAL_SPEC/tasks.md")
...
assert_true "accepts second sequential partial-story work unit" validate_iteration_commit_history "$TMP_PARTIAL_REPO" "$second_head" "$second_state" "$second_ids" "$second_completed_ids" 0 "$TMP_PARTIAL_SPEC/tasks.md" "$TMP_PARTIAL_SPEC/progress.md" "$TMP_PARTIAL_SPEC/ralph-memory.md"
...
third_completed_ids=$(get_completed_task_ids "$TMP_PARTIAL_SPEC/tasks.md")
...
assert_true "accepts final sequential partial-story work unit" validate_iteration_commit_history "$TMP_PARTIAL_REPO" "$third_head" "$third_state" "$third_ids" "$third_completed_ids" 0 "$TMP_PARTIAL_SPEC/tasks.md" "$TMP_PARTIAL_SPEC/progress.md" "$TMP_PARTIAL_SPEC/ralph-memory.md"
...
reset_completed_ids=$(get_completed_task_ids "$TMP_PARTIAL_SPEC/tasks.md")
...
reject_output=$(validate_iteration_commit_history "$TMP_PARTIAL_REPO" "$reset_head" "$reset_state" "$reset_ids" "$reset_completed_ids" 0 "$TMP_PARTIAL_SPEC/tasks.md" "$TMP_PARTIAL_SPEC/progress.md" "$TMP_PARTIAL_SPEC/ralph-memory.md" 2>&1) && reject_status=0 || reject_status=$?
...
failed_state_completed_ids=$(get_completed_task_ids "$TMP_PARTIAL_SPEC/tasks.md")
...
failed_state_output=$(validate_iteration_commit_history "$TMP_PARTIAL_REPO" "$failed_state_head" "$failed_state_snapshot" "$failed_state_ids" "$failed_state_completed_ids" 7 "$TMP_PARTIAL_SPEC/tasks.md" "$TMP_PARTIAL_SPEC/progress.md" "$TMP_PARTIAL_SPEC/ralph-memory.md" 2>&1) && failed_state_status=0 || failed_state_status=$?
...
assert_eq "completed existing ids ignore stale checked before-state" "0" "$(count_completed_task_ids $'T001\nT002' "T001" "T001")"
