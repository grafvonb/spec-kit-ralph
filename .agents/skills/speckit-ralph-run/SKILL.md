...
5. **Launch orchestrator script in a visible terminal**:
   ...
   - The launched orchestrator resolves the installed `.specify/templates/ralph-memory.md`, creates or validates the feature's `ralph-memory.md` before task selection, and blocks malformed memory without invoking an agent
